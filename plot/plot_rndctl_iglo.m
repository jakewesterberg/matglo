...
ilo = baseline_correct([ squeeze(glo_spks.conv(i,2:end-1,find(glo_info.ilo_rndctl)-3)); ...
...
[m,l,u] = confidence_interval(double(ilo));
plot_ci(smooth(l,ss),smooth(u,ss),pres_epoch,[.5 0 0],.25)
iLocal = plot(pres_epoch,smooth(m,ss),'linewidth',2,'color',[.5 0 0]);


igo = baseline_correct([ squeeze(glo_spks.conv(i,2:end-1,find(glo_info.igo_rndctl)-3)); ...
